...
loadThemes = function(env = getOption("RAdamant")
	...
	Logger(message = "Store themes in the given environment", from = "loadThemes", line = 44, level = 1);
...
draw.x.axis = function(X
    ...
    Logger(message = "Number of axis ticks", from = "draw.x.axis", line = 4, level = 1);
    N.ticks = ifelse(toupper(theme.params[["x.ticks"]]) == "ALL", N, as.numeric(theme.params[["x.ticks"]]));
    if(is.null(base)) {
        base = 1:N;
        # Tick points
        Logger(message = "Tick points", from = "draw.x.axis", line = 8, level = 1);
        x.ticks = round(seq(1, N, len = min(N, N.ticks, na.rm = TRUE)));
		# Sample a subset of the labels
		Logger(message = "Sample a subset of the labels", from = "draw.x.axis", line = 10, level = 1);
		...
		if(is.null(xlabels)) {
			...
			# Tick points
			Logger(message = "Tick points", from = "draw.x.axis", line = 19, level = 1);
			x.ticks = round(seq(1, N, len = min(N, N.ticks, na.rm = TRUE)));
			xlabels = xlabels[x.ticks];
		}
    }
    # Default labels if null
    Logger(message = "Default labels if null", from = "draw.x.axis", line = 24, level = 1);
    if(is.null(xlabels)) {
        xlabels = apply.format(x.ticks, fmt = theme.params[["xlab.fmt"]]);
    }
    # Padding labels
    Logger(message = "Padding labels", from = "draw.x.axis", line = 28, level = 1);
    if(length(xlabels) < length(x.ticks))
        xlabels = c(xlabels, 1:(length(x.ticks)-length(xlabels)));
    # Add horisontal axis  lines
    Logger(message = "Add horisontal axis  lines", from = "draw.x.axis", line = 31, level = 1);
    abline(h = par("usr")[3], col = theme.params[["axis.col"]]);
	if(show.labels) {
		# Add prefix if required
		Logger(message = "Add prefix if required", from = "draw.x.axis", line = 34, level = 1);
		if(any(nchar(theme.params[["xlab.prefix"]]) > 0)) {
			xlabels = paste(theme.params[["xlab.prefix"]], xlabels, sep = "");
		}
		# Add suffix if required
		Logger(message = "Add suffix if required", from = "draw.x.axis", line = 38, level = 1);
		if(any(nchar(theme.params[["xlab.suffix"]]) > 0)) {
			xlabels = paste(xlabels, theme.params[["xlab.suffix"]], sep = "");
		}
		# Add x-axis (ticks but no labels)
		Logger(message = "Add x-axis (ticks but no labels)", from = "draw.x.axis", line = 42, level = 1);
		axis(side = 1
			, at = x.ticks
			, labels = FALSE
		...
		Logger(message = "Add rotated text labels", from = "draw.x.axis", line = 52, level = 1);
		adj = 0.5;
...
draw.y.axis = function(X
    ...
    Logger(message = "Add vertical axis line", from = "draw.y.axis", line = 10, level = 1);
    abline(v = par("usr")[side], col = theme.params[["axis.col"]]);
	if(show.labels) {
		# Add prefix if required
		Logger(message = "Add prefix if required", from = "draw.y.axis", line = 13, level = 1);
		...
		# Add suffix if required
		Logger(message = "Add suffix if required", from = "draw.y.axis", line = 20, level = 1);
		...
		Logger(message = "Add y-axis (ticks but no labels)", from = "draw.y.axis", line = 27, level = 1);
		axis(side = 2*side
			, col = theme.params[["axis.col"] ]
		...
		# Add rotated text labels
		Logger(message = "Add rotated text labels", from = "draw.y.axis", line = 37, level = 1);
		adj = 2-side;
...
draw.grid = function(X
    ...
    Logger(message = "Number of grid ticks", from = "draw.grid", line = 4, level = 1);
    N.ticks = ifelse(toupper(theme.params[["grid.vlines"]]) == "ALL", N, as.numeric(theme.params[["grid.vlines"]]));
    # Vertical lines coordinates
    Logger(message = "Vertical lines coordinates", from = "draw.grid", line = 6, level = 1);
    if(is.null(base)) {
    ...
    # Draw vertical lines
    Logger(message = "Draw vertical lines", from = "draw.grid", line = 12, level = 1);
    abline(v = grid.v, col = theme.params[["grid.col"]], lty=3);
    # Horisontal lines coordinates
    Logger(message = "Horisontal lines coordinates", from = "draw.grid", line = 14, level = 1);
    grid.h = seq(min(X, na.rm = TRUE), max(X, na.rm = TRUE), len = theme.params[["grid.hlines"]]);
    # Draw horisontal lines
    Logger(message = "Draw horisontal lines", from = "draw.grid", line = 16, level = 1);
...
draw.y.title = function(ytitle = ""
    ...
    if(any(nchar(ytitle) > 0)) {
        ...
        Logger(message = "Calculate coordinates", from = "draw.y.title", line = 12, level = 1);
